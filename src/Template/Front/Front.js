...
class Front extends Component {
  ...
  renderManagemantLink() {
    if (this.state.user) {
      if (
        settings.root_user_types.includes(this.state.user.profile.studentType)
      ) {
        return (
          <a href="/management">
            <div className="dropDown-item">管理系統</div>
          </a>
        );
      }
    }
  ...
  render() {
    ...
    if (this.state.logIn) {
      let a = (
        <Link to="#" onClick={this.logOut} className="item url">
          Log out
        </Link>
      );
      let name;
      ...
      state = (
        <Link to="/login" className="url">
          <div className="url-box">Login</div>
        </Link>
      );
    }
