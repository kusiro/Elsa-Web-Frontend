...
class contentEdit extends Component {
  ...
  setUsers() {
    ...
    if (this.state.TAs) {
      this.state.TAs.map(
        item => (ta_ids.push(item.id), ta_names.push(item.profile.name))
      );
      this.setState({ ta_ids, ta_names });
    }
  }
  ...
  handleSubmit = () => {
    ...
    ins
      .put(`courses/${course_id}/contents/${content_id}`, this.state)
      .then(res => {
        console.log(res);
      ...
      return (
        <div className="content-new-user" key={user.id}>
          <img
            ...
            alt=""
            className={`content-new-avater ${chosen}`}
            onClick={e => this.addTaToggle(user.id, user.profile.name, e)}
          />
          <div className="content-new-user-info">{user.profile.name}</div>
          <div className="content-new-user-info">
  ...
  render() {
    // let users;
    // if (this.state.users) {
    //   users = this.state.users.map(item => this.mapUser(item));
    // }

    return (
      <div className="content-new-bg">
        <div className="content-new-main">
          <div className="content-new-title">New A Content</div>
          <div className="content-new-form">
            <div className="content-new-input-group">
              <div className="input-group-right-40">
                ...
                <select
                  className="content-new-select"
                  onChange={e => this.handleChange('season', e)}
                >
                  <option value="">choose type</option>
                  <option id="Spring" value="Spring">
                    Spring
                  </option>
                  <option id="Fall" value="Fall">
                    Fall
                  </option>
                </select>
              ...
              {/* <div className="input-group-left-60">
                <div className="content-new-ta">
                  ...
                  <div className="content-new-ta-main">{users}</div>
                </div>
              </div> */}
            </div>
            <div>
              <input
                ...
                className="content-new-submit-btn"
                type="submit"
              />
            </div>
          </div>
